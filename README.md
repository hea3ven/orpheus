...
`orpheus doctor` checks local task state and reports safe repairs. It also identifies Orpheus-owned dedicated worktrees left behind for closed tasks. Plain doctor reports clean worktrees as removable and others as needing attention. `doctor --fix` removes only a clean, unlocked worktree after rechecking its task, repository, and deterministic identity. It never forces removal, changes dirty worktrees, or mutates state in dry runs.
