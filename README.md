...
- `make test-integration` runs fast, almost-E2E in-process CLI workflows in `internal/cli` using `cli_test` and shared fixtures, plus owner-local adapter contracts, including real Git, real Beads, compiled CLI, and child-process scenarios. It requires `git` and `bd` on `PATH` (some schema scenarios also skip unless `dolt` is available); the target fails early when `bd` is unavailable.
- Humans classify integration tests by the boundary they prove. `Workflow` tests compose a use case across packages with semantic collaborators, as in `TestIntegrationWorkflowTaskRunCompletionLeavesTaskAwaitingManualReview`. `AdapterContract` tests call the owning package's concrete filesystem or executable adapter directly, rather than invoking a higher-level consumer to retest it, as in `TestIntegrationAdapterContractPublicationCommitAndPushPreserveMessageAndRemoteRef`. `BinaryE2E` tests compile and start a binary because startup, packaging, exit handling, or recursive commands are part of the assertion, as in `TestIntegrationBinaryE2ETaskRunUsesSeparateTaskProposalSelection`.
- Integration sources have `//go:build integration`. Top-level names begin `TestIntegrationWorkflow`, `TestIntegrationAdapterContract`, or `TestIntegrationBinaryE2E`; all three still belong to one integration lane. The repository convention test rejects an omitted or multiply selected top-level test body. Lane membership comes from the build constraint, not the name filter alone.
...
- `make quality` reads `.quality.yml`, runs each test lane once, and derives test, coverage, and timing decisions from those executions. `make check` adds formatting, linting, and a CLI build without repeating either lane. Pull requests run the same read-only check against the synthetic merge commit.

See [testing guidance](docs/testing.md) for category examples, boundary review, focused coverage audits, prerequisites, quality policy updates, and pull-request behavior.
...
- [Review pipeline workflow](docs/2026-08-28_review_pipeline_workflow.md) explains how `task run` handles implementation, review, follow-up work, approval, and publication or finalization retry.
- [Publication titles](docs/2026-06-23_repo_publication_titles.md) explains machine-wide and repository-specific Jira-style commit and pull-request titles, precedence, defaults, and missing-task-reference recovery.
- [Task branch templates](docs/task_branch_templates.md) explains global and repository branch naming, placeholder normalization, precedence, and retry behavior.
...
`orpheus doctor` checks local task state and reports safe repairs. It also identifies Orpheus-owned dedicated worktrees left behind for closed tasks. Plain doctor reports eligible worktrees as `cleanup_pending` and directs you to `doctor --fix`; it does not check cleanliness or change files. After confirming closure, task metadata, local state, repository ownership, deterministic path, branch, and lock state, `doctor --fix` makes one native, non-forced `git worktree remove` attempt per eligible worktree and reports Git's result. Git allows ignored build and coverage outputs, such as `/orpheus` and `/artifacts/test-coverage/`, but refuses tracked or non-ignored untracked changes.

Automatic cleanup after pull-request or direct-merge closure uses the same checks and removal attempt. Cleanup failure leaves the task closed and reports the retained path and reason. Cleanup never forces removal or runs clean, stash, reset, or commit. It preserves task branches, local task state, logs, and backend records. Already absent worktrees are harmless; incomplete Git registrations need manual repair.
