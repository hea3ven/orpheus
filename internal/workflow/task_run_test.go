...
func TestSelectTaskRunRoute(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "interrupted implementation retries", task: task.Task{ID: "op-1"}, state: taskstate.TaskState{Runs: []taskstate.RunAttempt{{Attempt: 1, Status: taskstate.RunStatusInterrupted}}}, want: TaskRunActionStartImplementation},
		{name: "interrupted handoff starts review", task: task.Task{ID: "op-1"}, state: taskstate.TaskState{Runs: []taskstate.RunAttempt{{Attempt: 1, Status: taskstate.RunStatusInterrupted, Completion: &taskstate.Completion{Summary: "done"}}}}, want: TaskRunActionStartReview},
		...
		{name: "automated blocker decision remains paused", task: task.Task{ID: "op-1"}, state: taskstate.TaskState{Runs: []taskstate.RunAttempt{completedRun}, Reviews: []taskstate.ReviewAttempt{{Attempt: 1, Status: taskstate.ReviewStatusWaitingForAutomatedDecision, Step: "unit"}}}, want: TaskRunActionReviewPaused},
		...
		{name: "running review with undecided automated blocker enters recovery", task: task.Task{ID: "op-1"}, state: taskstate.TaskState{Runs: []taskstate.RunAttempt{completedRun}, Reviews: []taskstate.ReviewAttempt{{Attempt: 1, Status: taskstate.ReviewStatusRunning, Step: "check", Steps: []taskstate.ReviewStep{{Kind: taskstate.ReviewStepKindCheck, Name: "check"}}, Findings: []taskstate.ReviewFinding{{Type: taskstate.FindingTypeBlocking, Step: "check", Title: "fix", Description: "fix it"}}}}}, want: TaskRunActionStartReview},
