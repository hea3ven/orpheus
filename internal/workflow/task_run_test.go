...
func TestSelectTaskRunRoute(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "automated blocker decision resumes", task: task.Task{ID: "op-1"}, state: taskstate.TaskState{Runs: []taskstate.RunAttempt{completedRun}, Reviews: []taskstate.ReviewAttempt{{Attempt: 1, Status: taskstate.ReviewStatusWaitingForAutomatedDecision, Step: "unit"}}}, want: TaskRunActionResumeReview},
