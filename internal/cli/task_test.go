...
func TestIntegrationTaskRunRejectsClosedTaskBeforeLocalWorkflowRouting(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunExecutesImplementerDefaultAttachedFromDeterministicWorktree(t *testing.T) {
	...
	is.True(state.Runs[0].Execution.Interactive)
...
func TestIntegrationTaskRunStructuredPiProfileCapturesUsage(t *testing.T) {
	...
	is.False(execution.Interactive)
...
func TestIntegrationTaskRunRejectsMissingRequiredExternalReferenceBeforeSetup(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunRejectsChildWhenImmediateParentEpicIsNotInProgress(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunPlainRetryRequiresMainForRepoRootMetadata(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunRepoRootFailsDirtyRepoRootBeforeLaunch(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunMainAllowsOwnedInProgressRepoRootTask(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunMainBlocksOtherRepoRootOwnerButWorktreeRunStillWorks(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunWorktreeModeDoesNotCareAboutDirtyRepoRoot(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunAllowsOwnedInProgressTaskWithMatchingMetadata(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunFailsFastWhenGlobalMutationLockHeldBeforeSetup(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunReleasesGlobalMutationLockWhileAgentRunsAndReacquiresForFinish(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunAgentFlagSelectsNamedProfile(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunRecordsFailedAttemptWhenAgentExitsNonZero(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunRecordsStartFailureWhenAgentProcessDoesNotStart(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunHeaderWriteFailureRecordsStartFailure(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunRefusesLatestRunningAttempt(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskRunReportsUnknownAgentProfileBeforeLaunching(t *testing.T) {
	t.Parallel()
	is := assert.New(t)
	must := require.New(t)
	root := newTestState(t)
...
func TestIntegrationTaskReviewCheckBlockerReasonEOFRecordsInterrupted(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			recordMainCompletion(t, paths, "alpha", "op-main", repoPath, "Review reason EOF", "Interrupt blocker reason.")
			must.NoError(os.WriteFile(filepath.Join(repoPath, "reviewed.txt"), []byte("reviewed\n"), 0o644))
...
func TestIntegrationTaskReviewCheckBlockerKeepAcceptsEOFAnswer(t *testing.T) {
	...
	recordMainCompletion(t, paths, "alpha", "op-main", repoPath, "Review keep EOF", "Keep blocker without newline.")
	must.NoError(os.WriteFile(filepath.Join(repoPath, "reviewed.txt"), []byte("reviewed\n"), 0o644))
...
func TestIntegrationTaskReviewCheckBlockerReasonAcceptsEOFAnswer(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			recordMainCompletion(t, paths, "alpha", "op-main", repoPath, "Review reason EOF", "Accept blocker reason.")
			must.NoError(os.WriteFile(filepath.Join(repoPath, "reviewed.txt"), []byte("reviewed\n"), 0o644))
...
func TestIntegrationTaskRunAfterInterruptedAutomatedBlockerDecisionRequiresFreshReview(t *testing.T) {
	...
	recordMainCompletion(t, paths, "alpha", "op-main", repoPath, "Review interrupted", "Do not launch follow-up.")
	must.NoError(os.WriteFile(filepath.Join(repoPath, "reviewed.txt"), []byte("reviewed\n"), 0o644))
	...
	agentLogPath := withFakeAgent(t, "followup-agent", 0)
...
func TestIntegrationTaskReviewInterruptedAutomatedBlockerRecoveryReusesRecordedPipeline(t *testing.T) {
	...
	must.NoError(store.Save(registry.Registry{Repos: []registry.Repo{{
		...
		BeadsMode:      registry.BeadsModeLocal,
		BeadsPrefix:    "op",
		ReviewPipeline: "default",
	}}}))
	recordMainCompletion(t, paths, "alpha", "op-main", repoPath, "Review strict recovery", "Recover with selected pipeline.")
	must.NoError(os.WriteFile(filepath.Join(repoPath, "reviewed.txt"), []byte("reviewed\n"), 0o644))

	defaultCheck := writeReviewScript(t, "#!/bin/sh\nprintf 'default pipeline\\n'\nexit 7\n")
	strictCheck := writeReviewScript(t, "#!/bin/sh\nprintf 'strict pipeline\\n'\nexit 7\n")
	writeReviewPipelineConfig(t, paths, "default", map[string][]map[string]any{
...
func writeAutonomousReviewLoopConfig(
	...
	require.NoError(t, paths.WriteConfigYAML(agent.ConfigFile, map[string]any{
		"agents": map[string]any{
			...
			"profiles": map[string]any{
				implementerName: map[string]any{"command": implementerCommand, "interactive": false},
			},
		},
		"reviews": map[string]any{
			"default_pipeline":               "standard",
...
func writeAutonomousReviewLoopConfigWithImplementers(
	...
	require.NoError(t, paths.WriteConfigYAML(agent.ConfigFile, map[string]any{
		"agents": map[string]any{
			...
			"profiles": map[string]any{
				firstName:  map[string]any{"command": firstCommand, "interactive": false},
				secondName: map[string]any{"command": secondCommand, "interactive": false},
			},
		},
		"reviews": map[string]any{
			"default_pipeline":               "standard",
