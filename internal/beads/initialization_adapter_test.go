...
func TestIntegrationAdapterContractInitializeManagedCreatesUsableIsolatedDatabase(t *testing.T) {
...
func TestIntegrationAdapterContractInspectLocalFindsExistingUsableDatabaseWithoutReinitialization(t *testing.T) {
	binary, err := exec.LookPath("bd")
	require.NoError(t, err, "bd is required for the integration lane")
	root := testutil.CanonicalTempDir(t)
	runner := isolatedBeadsRunner(t, binary)
